...
var fs = require("fs");
var ffmpeg = require('fluent-ffmpeg');//ffmpeg调用
var filesize = require('file-size');//文件大小数据格式化
...
//
//这里所有的请求，实际上都加上了/users
//

router.get("/admin/addChannel", function (req, res) {
    if (req.session.user.identify == "允许直播") {//获取所有允许直播的频道
...
router.get("/account", function (req, res) {
    method.method.users
        ...
        .populate({path: "fans follows", select: "name face _id regTime uid"})
...
router.put("/account", function (req, res) {
    ...
    method.method.users
        .findByIdAndUpdate(req.session.user._id, req.body)
        .exec(function (err, cb) {
            console.log(cb);
            res.json(err)
        })
...
router.post("/uploadPoster", function (req, res) {
    var file = req.files;
    console.log(file);
    var appPath = process.cwd();//此文件地址
    var path = appPath + "/public/admin/" + req.session.user.name + "/" + file.file.name + ".jpg";//保存地址
    ffmpeg()//图片转码
...
router.post("/channel", function (req, res) {
    ...
    _channel
        .save(function (err, cb) {
            if (!err) {
                method.method.users
                    .findByIdAndUpdate(req.session.user._id, {channels: _channel._id})
                    .exec()
            }
            res.json(err)
        })
...
router.post("/video", function (req, res) {
    ...
    _video.save(function (err, cb) {
        if (!err) {
            method.method.users
                .findByIdAndUpdate(req.session.user._id, {$push: {videos: _video._id}})
                .exec()
            res.json(err)
        }
    })
...
router.delete("/video/:_id", function (req, res) {
    ...
    method.method.videos
        ...
        .exec(function (err, cb) {
            console.log(cb)
            method.method.users
                .findByIdAndUpdate(req.session.user._id, {$pull: {videos: _id}})
                .exec(function (err, cb) {
                    console.log(err);
                    res.json(err)
                })
...
router.get("/saveFace", function (req, res) {
    if (req.session.face) {
        method.method.users
            .findByIdAndUpdate(req.session.user._id, {face: req.session.face})
            .exec(function (err, cb) {
                res.json(err);
...
router.post("/addComment/:_id", function (req, res) {
    ...
    console.log(data);
    method.method.videos
        .findByIdAndUpdate(req.params._id, {$push: {comments: data}})
        .exec(function (err, cb) {
            method.method.videos
                .findById(req.session.videoId)
                .populate({path: "comments.user", select: "name uid face fans follows regTime channels videos weibo"})
                .select("comments")
                .exec(function (err, cb2) {
...
router.put("/password", function (req, res) {
    ...
    console.log(data)
    method.method.users
        .findOne({_id: req.session.user._id, password: data.oldPwd})
        .exec(function (err, cb) {
            if (cb) {
                method.method.users
                    .findByIdAndUpdate(req.session.user._id, {password: data.Pwd})
                    .exec(function (err2, cb2) {
                        console.log(cb2)
                        res.json(err2)
                    })
            }
...
router.put("/addFollow", function (req, res) {
    method.method.users
        .findOne({_id: req.session.user._id, follows: req.session.bc})
        .exec(function (err, cb) {
            if (!cb) {
                method.method.users
                    .findByIdAndUpdate(req.session.user._id, {$push: {follows: req.session.bc}})
                    .exec()
                method.method.users
                    .findByIdAndUpdate(req.session.bc, {$push: {fans: req.session.user._id}})
                    .exec()
            }
            res.json(cb)
        })
