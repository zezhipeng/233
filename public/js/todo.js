...
todo.controller("liveCtrl", function ($scope, $http, $sce, api, req, $location,rest,$resource) {
    ...
    $scope.addFollow = function (channels) {
        $http.put("/users/addFollow", {channels: channels}).success(function (res) {
            if (!res) {
                alert("添加成功")
            }
            else {
...
todo.controller("accountCtrl", function ($scope, $http, req) {
    ...
    $scope.remove = function (_id) {
        $http.delete("/users/video/" + _id).success(function (res) {
            $scope.user.videos = [];
            $http.get("/users/video").success(function (res) {
    ...
    $scope.save = function () {
        console.log($scope.user);
        $http.put("/users/account", $scope.user).success(function (res) {
            if (!res) {
                alert("保存成功")
            }
        })
...
todo.controller("videoCtrl", function ($scope, $http, req,$resource,rest) {
    $scope.comments=null;$scope.comment=null;
    rest.get({collection:"videos"})
        .$promise.then(function(res){
        $scope.videos = res
    });

    $resource("/comments/:_id",{_id:"@id"},{"get":{method:"get",isArray:true}})
        .get({_id:videoId})
        .$promise.then(function(res){
        $scope.comments = res
    });
...
todo.controller("channelCtrl", function ($scope, $http) {
    $scope.channel=null;
    $http.get("/users/myChannel").success(function (res) {
        console.log(res);
        $scope.channel = res
    })
